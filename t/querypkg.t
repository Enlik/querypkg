...
# some simple tests with make_URI
$c->set_req_params(
	arch => 'x86', type => 'pkg', order => 'downloads', repo => 'p');
my $got = $c->make_URI( "haha" );
...
$got = $c->make_URI( '@set' );
like( $got, qr/\?q=%40set&/, "URI: keyword ok (\@keyword)" );
...
$got = $c->make_URI( '@set' );
like( $got, qr/\?q=\@set&/, "URI: keyword ok (type set, \@keyword)" );
...
is( $c->package_name_check( "" ), App::Querypkg::Core::PKG_NAME_TOO_SHORT,
	"package_name_check() with empty value");
